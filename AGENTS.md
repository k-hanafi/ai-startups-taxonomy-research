...
Last updated: 2026-07-12 · Active branch: `cursor/rename-two-pass-to-classification-c7d5` (rename `two-pass` → `classification` for CLI clarity; next = paid Stage 8 9-cell)
...
- `.cursor/plans/golden_set_eval_harness_*.plan.md` — golden-set eval harness (active; classification committed; #22 dashboard + #24/#25 science on `main`; provisional `draft_*` gold accepted for paid sweep; Stage 8 next = paid 9-cell with Pass A auto-banked once per model).
...
| `__main__.py` | CLI: `run-classification` (Stage 8), `matrix` (9-cell dry-run), `score`, `dashboard`; legacy `run` warns and is not the Stage 8 path |
...
| `data visualization/02_Analysis_Code/build_eval_dashboard.py` | Stage 9 eval viewer: Pareto / leaderboard / ECE+selective / reliability / vs_baseline / latency + config filter. Defaults to mock fixture; `--runs`/`--scored` for real runs. |
...
python -m evals run-classification --model gpt-5.4-nano --effort-b low --require-stage8-cell
...
# legacy: python -m evals run  (single-pass; retired for Stage 8; warns)```
...
| Eval Stage 8 matrix / scoring | `evals/config.py` (`EVAL_MODELS` + `STAGE8_PASS_B_EFFORTS`); `run-classification` (auto Pass A bank); `matrix`; `score --confidence-from-raw [--baseline]` |
